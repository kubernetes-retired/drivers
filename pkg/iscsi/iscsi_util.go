...
func (util *ISCSIUtil) AttachDisk(b iscsiDiskMounter) (string, error) {
	...
	for _, tp := range bkpPortal {
		...
		if err != nil {
			klog.Errorf("iscsi: failed to rescan session with error: %s (%v)", string(out), err)
		}
		...
		if exist := waitForPathToExist(&devicePath, 1, iscsiTransport); exist {
			klog.V(4).Infof("iscsi: devicepath (%s) exists", devicePath)
	...
	if err := util.persistISCSI(*(b.iscsiDisk), b.targetPath); err != nil {
		klog.Errorf("iscsi: failed to save iscsi config with error: %v", err)
		return "", err
	}
